...
| ImportError: libcblas.so.3: cannot open shared object file: No such file or directory | You may need to install extra dependencies. Google the error message for your specific operating system |
...
Once the script is running on your device, you can request an image by sending an email to the configured email address (`address` in the config) from one of the approved email addresses (`approved_users`) with the one-time password generated by your authenticator app as the body of the email.
The email does not need a subject.

You will need to wait a *maximum* of `polling_period` seconds before you receive a response from the Monitor.
This will be a new email (not a reply to the original thread) containing an image of the printer.

If you want to close the terminal that is running the script, on Linux operating systems you can press `Ctrl`+`Z` and type `disown -h`. 
