...
def check_mail(mail:imaplib.IMAP4_SSL, config:dict):
    ...
    for id_ in data[0].split():
        ...
        success, data = mail.fetch(id_, '(BODY[HEADER.FIELDS (SUBJECT)] BODY[HEADER.FIELDS (FROM)] BODY[1])')
        # Skip if success!='OK'
        if success != 'OK':
        ...
        emails.append({"address": from_address, "subject": subject, "body": message_body})
...
def clear_large_mail(mail:imaplib.IMAP4_SSL, config:dict):
    ...
    result, data = mail.search(None, "(NOT SMALLER 10000 TO \"{}\")".format(config['credentials']['address']))
